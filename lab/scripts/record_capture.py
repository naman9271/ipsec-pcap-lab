...
PROFILE={1:('tunnel','IKEv2','IPv4','off','no','172.30.0.2','172.30.0.3'),2:('tunnel','IKEv2','IPv4','on','yes','172.30.0.2','172.30.0.3'),3:('tunnel','IKEv1','IPv4','off','no','172.30.0.2','172.30.0.3'),4:('transport','IKEv1','IPv4','on','yes','172.30.0.2','172.30.0.3'),5:('tunnel','IKEv2','IPv6','off','no','fd00:30::2','fd00:30::3')}
def version(cmd):
 try: return subprocess.check_output(cmd, text=True, stderr=subprocess.STDOUT).splitlines()[0]
 except Exception: return 'unavailable'
p=argparse.ArgumentParser(); p.add_argument('pcap'); p.add_argument('--label',required=True); p.add_argument('--profile',type=int,required=True); p.add_argument('--run',default=''); p.add_argument('--role',default='train_known'); p.add_argument('--generator',required=True); p.add_argument('--params',default='{}'); p.add_argument('--anomaly',default=''); p.add_argument('--interface',default='host-veth'); a=p.parse_args()
params=json.loads(a.params); info=inspect(a.pcap); mode,ike,ip,natt,forced,left,right=PROFILE[a.profile]
...
row={x:'' for x in FIELDS}; row.update(sample_id=sid,pcap_file=rel,traffic_class=a.label,mode=mode,ike_version=ike,ip_version=ip,nat_t=natt,nat_t_forced=forced,actual_nat_present='no',peer_auth='PSK',capture_duration_s=str(params.get('duration_s','')),outer_left=left,outer_right=right,traffic_generator=a.generator,sha256=info['sha256'],original_label=a.label,canonical_label=canonical,dataset_role=a.role,profile_id=f'P{a.profile:02d}',run_id=a.run,capture_timestamp_utc=dt.datetime.now(dt.timezone.utc).isoformat(),configured_capture_duration_s=str(params.get('duration_s','')),observed_packet_span_s=str(info['observed_packet_span_s']),packet_count=str(info['packet_count']),captured_bytes=str(info['captured_bytes']),capture_interface=a.interface,link_type=str(info['link_type']),generator_parameters=json.dumps(params,sort_keys=True),strongswan_version=version(['docker','exec','ipsec-left','ipsec','--version']),kernel_version=platform.release(),tcpdump_version=version(['tcpdump','--version']),is_anomaly=str(bool(a.anomaly)).lower(),anomaly_type=a.anomaly or 'none')
rows=[]
if meta.exists():
